...
const Main = (props) => {
  ...
  const keywordLists = keywordList.map((keywords, index) => {
    return (
      <h3
        key={index}
        onClick={() => props.handleTagClick(keywords)}
        className="keyword"
      >
        {keywords}
      </h3>
            ...
            <div className="info-container">
              ...
              <div className="info">
                ...
                <div className="period">
                  <span className="date">{props.postedAt}</span> &bull;{" "}
                  <span className="type">{props.contract}</span> &bull;{" "}
                  <span className="country">{props.location}</span>
                </div>
              </div>
            </div>

            <div className="keywords">{keywordLists}</div>
          </div>
        </div>
      </section>
    </div>
